    ...
    messages: [
      {
        ...
        content: [
          {
            type: 'text',
            text: 'Transcribe the spoken English exactly. Output only the transcript. Do not add explanations or timestamps. If no speech is audible, output an empty string.',
          },
          {
            type: 'input_audio',
            input_audio: { data: readFileSync(process.argv[2]).toString('base64'), format: 'wav' },
          },
        ],
      },
    ],
...
console.log(
  JSON.stringify({
    ...
    pass,
    transcript,
    error: body.error?.message?.replaceAll(key, '[redacted]'),
  }),
);
