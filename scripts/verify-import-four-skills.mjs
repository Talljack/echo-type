...
const browser=await chromium.launch({args:['--use-fake-ui-for-media-stream','--use-fake-device-for-media-stream',`--use-file-for-fake-audio-capture=${audio}`]});
try {
 const context=await browser.newContext({permissions:['microphone']});
 const page=await context.newPage();page.setDefaultTimeout(30000);
 await page.addInitScript(({simulate})=>{
  Object.defineProperty(window,'SpeechRecognition',{value:undefined,configurable:true});
  Object.defineProperty(window,'webkitSpeechRecognition',{value:undefined,configurable:true});
  if(simulate)Object.defineProperty(window.speechSynthesis,'speak',{value:u=>setTimeout(()=>u.onend?.(),100)});
 ...
 await page.route('**/api/stt',async route=>{
  ...
  const audio=form.get('audio');assert.ok(audio.size>100);
  const f=new FormData();f.append('file',audio,'recording.webm');f.append('response_format','verbose_json');f.append('language','en');
  const response=await fetch('http://127.0.0.1:8178/inference',{method:'POST',body:f});
  const j=await response.json();assert.equal(response.status,200);assert.match(j.text,/application|response time|index/i);
  transcripts.push(j.text.trim());await route.fulfill({json:{text:j.text}});
 });
 ...
 console.log(JSON.stringify({listen:true,tts:process.env.SIMULATE_TTS==='1'?'simulated completion event':'real browser synthesis completion'}));
