  ...
  for (const type of ['sentences', 'scenario']) {
    ...
    await page.route('**/api/import/transcribe', async (route) => {
      ...
      assert.equal(
        ...
        'Test transport must retain all uploaded audio bytes',
      );
    ...
    await page
      ...
      .fill('An index improved the response time.');
    ...
    await page.reload();
    assert.equal(
      await page.getByRole('textbox', { name: 'Your response', exact: true }).inputValue(),
      'An index improved the response time.',
    );
        ...
        new Promise((resolve, reject) => {
          ...
          request.onsuccess = () => {
            const db = request.result,
              tx = db.transaction('learningAttempts', 'readwrite');
            const store = tx.objectStore('learningAttempts'),
              all = store.getAll();
            all.onsuccess = () => {
              for (const a of all.result)
                store.put({ ...a, createdAt: a.createdAt - 172800000, updatedAt: a.updatedAt - 172800000 });
            };
            tx.oncomplete = () => {
              db.close();
              resolve();
    ...
    await page.getByRole('button', { name: '4. Recall', exact: true }).click();
    await page
      .getByRole('textbox', { name: 'Recall from memory' })
      .fill('The team added an index to improve response time.');
    ...
    await page
      ...
      .fill('Our customer support response time improved after we shared a troubleshooting guide.');
    ...
    console.log(
      JSON.stringify({
        ...
        verified: 'real audio → real AI organization → review/tag → publish → all five stages → reload',
      }),
    );
    await context.close();
  }
