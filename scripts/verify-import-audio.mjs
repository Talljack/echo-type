...
for (const ext of ['wav', 'mp3', 'm4a', 'ogg', 'flac', 'webm', 'mp4', 'avi']) {
  ...
  execFileSync('ffmpeg', ['-y', '-i', path.join(dir, 'source.aiff'), '-ar', '16000', '-ac', '1', '-c:a', codec, file], {
    stdio: 'ignore',
  });
  const form = new FormData();
  form.set('file', new File([readFileSync(file)], `sample.${ext}`));
  form.set('provider', 'openrouter');
  form.set('language', 'en');
  try {
    const res = await fetch('http://127.0.0.1:3018/api/import/transcribe', {
      method: 'POST',
    ...
    console.log(
      JSON.stringify({
        ...
        text: ok ? body.text : undefined,
        error: ok ? undefined : String(body.error).replaceAll(key, '[redacted]'),
      }),
    );
    if (!ok) failed++;
  } catch (e) {
    failed++;
    console.log(JSON.stringify({ format: ext, pass: false, error: e.name }));
  }
}
