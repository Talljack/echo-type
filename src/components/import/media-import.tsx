...
import {
  Link2, Mic, Loader2, AlertCircle, Download,
} from 'lucide-react';
  ...
  const classifyContent = async (text: string, contentTitle: string) => {
    ...
    try {
      ...
      const res = await fetch('/api/tools/classify', {
        method: 'POST', headers,
        body: JSON.stringify({ text, title: contentTitle, provider: activeProviderId, modelId: activeConfig.selectedModelId, baseUrl: activeConfig.baseUrl || PROVIDER_REGISTRY[activeProviderId].baseUrl, apiPath: activeConfig.apiPath || PROVIDER_REGISTRY[activeProviderId].apiPath }),
      });
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-2 mb-2">
        <Button
          ...
          className={importMode === 'url' ? 'bg-indigo-600 cursor-pointer' : 'border-indigo-200 text-indigo-600 cursor-pointer'}
        >
          <Link2 className="w-4 h-4 mr-1" />URL Import
        </Button>
        <Button
          variant={importMode === 'local' ? 'default' : 'outline'}
          size="sm"
          onClick={() => setImportMode('local')}
          className={importMode === 'local' ? 'bg-indigo-600 cursor-pointer' : 'border-indigo-200 text-indigo-600 cursor-pointer'}
        >
          <Mic className="w-4 h-4 mr-1" />Local Upload
        </Button>
      </div>
