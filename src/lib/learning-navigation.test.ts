...
describe('learning navigation', () => {
  it('exposes six core destinations and two specialist destinations', () => {
    expect(PRIMARY_LEARNING_LINKS.map((link) => link.href)).toEqual([
      '/dashboard', '/learn', '/library', '/resources', '/review', '/favorites', '/speak', '/pronunciation',
    ]);
  });
  it.each([
    ['/journal', 'notes'], ['/favorites', 'notes'], ['/favorites/review', 'review'],
    ['/review/today', 'review'], ['/weak-spots', 'review'], ['/library/wordbooks/a', 'materials'], ['/resources', 'resources'],
